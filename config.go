...
import (
	...
	"github.com/kpelzel/mixoder/util"
)
...
const (
	...
	configKeyProcPriority        = "proc_priority"
	configKeyInvertControls      = "invert_controls"
...
func NewConfig(logger *zap.SugaredLogger, notifier Notifier) (*CanonicalConfig, error) {
	...
	userConfig.SetDefault(configKeyProcPriority, map[string]string{})
	userConfig.SetDefault(configKeyInvertControls, false)
...
// func priorityListFromConfigs(userMapping map[string]string, internalMapping map[string]string) *sliderMap {
// 	resultMap := newSliderMap()

// 	// copy targets from user config, ignoring empty values
// 	for priorityIdxString, targets := range userMapping {
// 		priorityIdx, _ := strconv.Atoi(sliderIdxString)

// 		resultMap.set(sliderIdx, funk.FilterString([]string{targets}, func(s string) bool {
// 			return s != ""
// 		}))
// 	}
...
// 		filteredTargets := funk.FilterString([]string{targets}, func(s string) bool {
